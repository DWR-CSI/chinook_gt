...
clean_sample_name <- function(x) {
  x %>%
    str_remove("_S\\d+.*") %>%  # Remove _S followed by any number and everything following it
    str_replace_all("-", "_")    # Replace any dashes with underscores (if any)
}
...
raw_repunit_probs <- all_full_mix_results %>%
  ...
  mutate(
    tributary = case_when(
      (RoSA == "Early") & (repunit %in% c("fall", "latefall")) & ((n_miss_loci / (n_miss_loci + n_non_miss_loci)) < gsi_missing_threshold) ~ "Feather River Lineage Spring",
